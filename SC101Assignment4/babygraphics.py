    ...
    for i in range(len(lookup_names)):
        ...
        if name in name_data:
            ...
            for j in range(len(YEARS)-1):
                ...
                rank1 = int(years_data.get(year1, MAX_RANK))
                rank2 = int(years_data.get(year2, MAX_RANK))
                ...
                canvas.create_text(x1 + TEXT_DX, y1, text = f"{name}{rank1 if year1 in years_data else ''}", anchor = tkinter.SW, fill=color)
                x_last = get_x_coordinate(CANVAS_WIDTH, len(YEARS)-1)
                rank_last = int(years_data.get(str(YEARS[-1]),MAX_RANK))
                y_last = GRAPH_MARGIN_SIZE + (CANVAS_HEIGHT - 2 * GRAPH_MARGIN_SIZE) * rank_last/MAX_RANK
                canvas.create_text(x_last + TEXT_DX,y_last,text = f"{name}{rank_last if str(YEARS[-1]) in years_data else ''}", anchor = tkinter.SW, fill = color)
